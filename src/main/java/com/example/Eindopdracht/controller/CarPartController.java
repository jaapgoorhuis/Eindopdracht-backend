...
public class CarPartController {
    ...
    public ResponseEntity<Object> updateCarPart(@PathVariable Long id, @Valid @RequestBody CarPartInputDto dto) {
        try {
            CarPartDto carPart = service.updateCarPart(id, dto);
            return new ResponseEntity<>(carPart, HttpStatus.CREATED);
        } catch(RecordNotFoundException re) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Carpart id not found");
        }
    }
